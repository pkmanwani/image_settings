...
json_file_path = '../beamlines/awa/2025_02_20/QuadScan2/parameters.json'
...
rx_mm_smooth = rx_mm
ry_mm_smooth = ry_mm
quad_values_smooth = quad_values
...
kval_x = kval_x[3:]
kval_y = kval_y[3:]
rx_mm_smooth = rx_mm_smooth[3:]
ry_mm_smooth = ry_mm_smooth[3:]
...
plot_path = os.path.join(output_dir, 'quad_scan_plot.png')
plt.savefig(plot_path)
print(f"Plot saved at: {plot_path}")

# Save Twiss parameter results in a text file
results_path = os.path.join(output_dir, 'quad_scan_results.txt')
with open(results_path, 'w') as f:
    f.write('========================================\n')
    f.write(f'enx: {enx * 1e6:.3f} mm mrad\n')
    f.write(f'eny: {eny * 1e6:.3f} mm mrad\n')
    f.write('========================================\n')
    f.write(f'betax at initial position: {beta_x:.3f} m\n')
    f.write(f'betay at initial position: {beta_y:.3f} m\n')
    f.write(f'alphax at initial position: {alpha_x:.3f}\n')
    f.write(f'alphay at initial position: {alpha_y:.3f}\n')
    f.write('========================================\n')
